...
// SECTION 1: ADD THESE NEW IMPORTS (LINE 5-10)
// ============================================
...
const app = express();
// SECTION 2: REPLACE YOUR EXISTING SESSION CONFIG (LINE 7-12)
// ============================================
app.use(express.json({ limit: '10mb' }));
app.use(express.urlencoded({ extended: true, limit: '10mb' }));

// Secure session configuration - REPLACE your existing session code
// Replace your existing session config with this:
app.use(session({
...
// SECTION 3: ADD SECURITY MIDDLEWARE (AFTER SESSION, BEFORE STATIC)
// ============================================
...
// Request sanitization middleware - prevents XSS attacks
...
// SECTION 4: SECURE STATIC FILE SERVING - REPLACE LINE 14
// ============================================
...
// SECTION 5: SECURE FILE UPLOAD - REPLACE YOUR MULTER CONFIG (LINE 16-25)
// ============================================
...
// File filter for uploads - only allow images
// File filter for uploads - only allow images
const fileFilter = (req, file, cb) => {
  // Sirf image types allow karo
...
// SECTION 6: ADD PASSWORD HASHING FUNCTIONS (AFTER YOUR EXISTING FUNCTIONS)
// Find your existing hashPassword function (around line 150) and REPLACE IT
// ============================================
...
async function comparePassword(password, hash) {
 ...
 // ============================================
// CATEGORY HELPER FUNCTIONS - ADD THIS SECTION
// ============================================
...
// ============================================
// ADMIN HELPER FUNCTIONS (ADD HERE)
// ============================================
    ...
    <div style="display:flex;min-height:calc(100vh - 70px);">
      <div style="width:250px;background:white;border-right:1px solid #e9ecef;padding:20px 0;position:sticky;top:70px;height:calc(100vh - 70px);overflow-y:auto;">
        ...
        <div style="padding:20px 0;">
          ...
          <!-- ADD THE CATEGORIES LINK HERE - BETWEEN Sliders and Customers -->
...
// Helper function to get header HTML (ENHANCED with cart count)
function getHeader(req) {
  ...
  // FIXED: Use req.session.userName instead of someObject
...
// Helper function to get footer HTML (UPDATED)
function getFooter() {
  // Load settings
  let settings = {};
  try {
  ...
  return `
    ...
    <script>
      ...
      window.onload = function() {
        ...
        // Use saved theme or fall back to settings
...
app.get("/", (req, res) => {
  ...
  const reviews = loadReviews(); // ADD THIS LINE - load reviews
  ...
  // Get new arrivals (manually selected)
  const newArrivals = products
    .filter(p => p.showInNewArrivals && p.active !== false)
    .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
    .slice(0, 8);

  // Get best sellers (manually selected)
...
<div class="categories-section">
  ...
  <div class="categories-grid">
    ${['Running', 'Boxing', 'Wrestling', 'Cricket', 'Football', 'Basketball'].map(category => {
      // Define icon and color for each category
      const categoryIcons = {
        'Running': '🏃',
        'Boxing': '🥊',
        'Wrestling': '🤼',
...
<div class="products-horizontal-grid">
  ...
  <div class="product-card-modern">
    ...
    <div class="product-info-modern">
      ...
      <!-- Add brand display if available -->
      ${product.brand ? `
  ...
  <script>
    ...
    document.querySelector('.newsletter-form')?.addEventListener('submit', function(e) {
      ...
      .then(data => {
        ...
        // New filter and quick view functions
...
// ============================================
// SECTION 9: UPDATE ADMIN LOGIN ROUTE (FIND THIS ROUTE - around line 170)
// REPLACE YOUR EXISTING /login POST route with this
// ============================================
...
app.post("/login", authLimiter, async (req, res) => {
  ...
  // Check in users.json for other admin users
  ...
  if (user) {
    ...
    if (user.password.startsWith('$2a$') || user.password.startsWith('$2b$')) {
      ...
      isValidPassword = (user.password === Buffer.from(password).toString('base64'));
    }
...
// ============================================
// ============================================
// ADD/EDIT PRODUCT PAGE (WITH SUBCATEGORY SUPPORT) - COMPLETE FIXED VERSION
// ============================================
app.get("/add-product", (req, res) => {
  ...
  const posts = loadPosts();
  const categories = loadCategories(); // Make sure this function exists
  let product = null;
...
<head>
  ...
  <style>
    ...
    .size-stock-table .in-stock {
      color: #28a745;
      font-weight: 600;
    }
    
    .size-stock-table .out-of-stock {
      color: #dc3545;
      font-weight: 600;
    }
...
<body>
  ...
  <div class="form-group">
    <label class="form-label">Show in New Arrivals</label>
    <div class="toggle-switch">
      <label class="switch">
...
<div class="form-row">
  ...
  <div class="form-group">
      ...
      <span>Active (visible to customers)</span>
        ...
        <div class="category-container">
          <div class="category-header">
            <h3 style="margin: 0;"><i class="fas fa-tags"></i> Category & Subcategory</h3>
          </div>
          
          <div class="category-tabs">
            <button type="button" class="category-tab active" onclick="showCategoryMode('select')">Select Existing</button>
            <button type="button" class="category-tab" onclick="showCategoryMode('new')">Add New Category</button>
          </div>
          
          <!-- Select Existing Category Mode -->
          <div id="selectCategoryMode">
            <div class="category-select-group">
              <div>
                <label class="form-label">Main Category</label>
                <select name="category_select" id="mainCategorySelect" class="form-select" onchange="loadSubcategories()">
                  <option value="">Select Main Category</option>
                  ${categories.map(cat => `
                  <option value="${cat.name}" data-id="${cat.id}" ${product && product.category === cat.name ? 'selected' : ''}>
                    ${cat.name}
                  </option>
                  `).join('')}
                </select>
    ...
    <select name="subcategory_select" id="subcategorySelect" class="form-select" onchange="updateSubcategory()">
        ...
        ` : ''}
            ...
            <div id="subcategoryQuickList" class="subcategory-container" style="display: none;">
              <label class="form-label">Quick Select Subcategory:</label>
              <div class="subcategory-list" id="subcategoryList"></div>
            </div>
          </div>
          
          <!-- Add New Category Mode -->
          <div id="newCategoryMode" style="display: none;">
            <div class="new-category-input">
              <input type="text" id="newCategoryInput" class="form-input" placeholder="Enter new category name">
              <button type="button" class="btn btn-success" onclick="addNewCategory()">Add Category</button>
            </div>
            
            <div class="category-suggestions">
              <span class="category-suggestion" onclick="suggestCategory('Running')">Running</span>
              <span class="category-suggestion" onclick="suggestCategory('Boxing')">Boxing</span>
              <span class="category-suggestion" onclick="suggestCategory('Wrestling')">Wrestling</span>
              <span class="category-suggestion" onclick="suggestCategory('Cricket')">Cricket</span>
              <span class="category-suggestion" onclick="suggestCategory('Football')">Football</span>
              <span class="category-suggestion" onclick="suggestCategory('Basketball')">Basketball</span>
              <span class="category-suggestion" onclick="suggestCategory('Tennis')">Tennis</span>
              <span class="category-suggestion" onclick="suggestCategory('Swimming')">Swimming</span>
              <span class="category-suggestion" onclick="suggestCategory('Gym')">Gym & Fitness</span>
            </div>
            
            <div style="margin-top: 20px;">
              <label class="form-label">Add Subcategories for this Category:</label>
              <div class="new-category-input">
                <input type="text" id="newSubcategoryInput" class="form-input" placeholder="Enter subcategory name">
                <button type="button" class="btn btn-success" onclick="addSubcategoryToList()">Add Subcategory</button>
              </div>
              
              <div id="newSubcategoriesList" class="subcategory-list" style="margin-top: 15px; min-height: 40px;"></div>
              <input type="hidden" name="new_subcategories" id="newSubcategoriesHidden" value="">
            </div>
          </div>
          
          <!-- Hidden inputs for final category and subcategory -->
          <input type="hidden" name="category" id="finalCategoryInput" value="${product ? product.category || '' : ''}">
          <input type="hidden" name="subcategory" id="finalSubcategoryInput" value="${product ? product.subcategory || '' : ''}">
          <input type="hidden" name="category_id" id="categoryIdInput" value="${product ? product.categoryId || '' : ''}">
        </div>
       <div class="form-card">
  <form method="POST" action="/add-product" enctype="multipart/form-data" id="productForm">
    ${isEditMode ? `<input type="hidden" name="editIndex" value="${req.query.edit}">` : ''}
    
    <!-- Product Name and Brand Fields - ABHI YAHI PE HONGE -->
    <div class="form-row">
      <div class="form-group">
    ...
    <!-- ... rest of the form continues ... -->
        ...
        <div class="form-row">
          <!-- Price with Discount -->
          <div class="form-group">
            <label class="form-label required">Price & Discount</label>
            
            <div class="price-container">
              <div class="price-input-group">
                <span class="currency">₹</span>
                <input type="number" name="price" id="originalPrice" class="form-input price-field" 
                       placeholder="Original Price" step="0.01" min="0"
                       value="${product ? (product.originalPrice || product.price) : ''}" 
            ...
            <div class="discount-input-group">
              <select name="discount_type" id="discountType" class="discount-type" onchange="calculateFinalPrice()">
                <option value="none" ${!product || !product.discount ? 'selected' : ''}>No Discount</option>
                <option value="percentage" ${product && product.discount && product.discount.type === 'percentage' ? 'selected' : ''}>Percentage (%)</option>
                <option value="fixed" ${product && product.discount && product.discount.type === 'fixed' ? 'selected' : ''}>Fixed Amount (₹)</option>
              </select>
              
              <input type="number" name="discount_value" id="discountValue" class="discount-input" 
                     placeholder="Value" min="0" value="${product && product.discount ? product.discount.value : ''}"
                     onchange="calculateFinalPrice()">
            </div>
            
            <!-- Price Preview -->
            <div id="pricePreview" style="margin-top: 15px; padding: 15px; background: #f8f9fa; border-radius: 8px;">
              <div style="display: flex; align-items: center; gap: 15px; flex-wrap: wrap;">
                <span class="original-price" id="originalPriceDisplay">₹${product ? (product.originalPrice || product.price) : '0'}</span>
                <span class="discount-badge" id="discountBadge" style="${!product || !product.discount ? 'display: none;' : ''}">
                  ${product && product.discount ? (product.discount.type === 'percentage' ? product.discount.value + '% OFF' : '₹' + product.discount.value + ' OFF') : ''}
                </span>
                <span class="final-price" id="finalPriceDisplay">₹${product ? product.price : '0'}</span>
              </div>
            </div>
          </div>
        ...
        <div class="sizes-section">
          <div class="sizes-header">
            <h3 style="margin: 0;"><i class="fas fa-tshirt"></i> Size-wise Stock Management</h3>
            <button type="button" class="btn btn-success" onclick="addNewSizeRow()">
          ...
          <table class="size-stock-table" id="sizeStockTable">
            ...
            <tbody id="sizeStockBody">
              ...
              <tr>
                <td>
                  <input type="text" name="size_names[]" value="${size}" class="form-input" style="width: 100px;" readonly>
                </td>
                <td>
                  <input type="number" name="size_stocks[]" value="${stock}" class="form-input" min="0" style="width: 100px;" onchange="updateSizeStatus(this)">
                </td>
                <td>
                  <span class="stock-badge ${stock > 0 ? 'badge-in-stock' : 'badge-out-stock'}">
                    ${stock > 0 ? 'In Stock' : 'Out of Stock'}
                  </span>
                </td>
                <td>
                  <button type="button" class="delete-size-btn" onclick="deleteSizeRow(this)">
          ...
          <div class="add-size-row">
            <input type="text" id="newSizeInput" class="add-size-input" placeholder="Enter size (e.g., S, M, L, XL)">
            <input type="number" id="newSizeStockInput" class="add-size-input" placeholder="Stock quantity" min="0" value="0">
            <button type="button" class="add-size-btn" onclick="addSizeWithStock()">
              <i class="fas fa-plus"></i> Add Size with Stock
            </button>
          </div>
          
          <div class="total-stock" id="totalStockDisplay">
            Total Stock: <span id="totalStockValue">${product ? (product.stock || 0) : 0}</span> units
          </div>
        </div>
        ...
        <div class="form-row">
          ...
          <div class="form-group">
            <label class="form-label">Overall Stock (Auto-calculated)</label>
            <input type="number" name="stock" id="overallStock" class="form-input" 
                   value="${product ? product.stock : '0'}" readonly>
          </div>
        </div>
        ...
        <div class="form-group">
          ...
          <div class="file-upload">
            <div class="file-upload-label" onclick="document.getElementById('imageInput').click()">
              ...
              <span style="color: #666; text-align: center;">
                Upload product images (JPG, PNG, GIF)<br>
          ...
          <div class="image-preview" id="imagePreview">
            ${product && product.images ? product.images.map(img => `
            <div style="position: relative;">
              <img src="${img}" class="preview-image">
              <input type="hidden" name="existingImages[]" value="${img}">
              <button type="button" style="position: absolute; top: 5px; right: 5px; background: #dc3545; color: white; border: none; border-radius: 50%; width: 25px; height: 25px; cursor: pointer;" onclick="removeExistingImage(this)">
                ×
              </button>
            </div>
            `).join('') : ''}
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Store categories data passed from the server
        const categories = ${JSON.stringify(loadCategories() || [])};
        let newSubcategories = [];

        // ===== INITIALIZATION =====
        console.log('Initializing product form...');
        updateTotalStock(); // Calculate total stock from existing size rows
        calculateFinalPrice(); // Set the initial price preview

        // Load subcategories if the product already has a category selected (for edit mode)
        const mainCategorySelect = document.getElementById('mainCategorySelect');
        if (mainCategorySelect && mainCategorySelect.value) {
            loadSubcategories();
        }

        // Set the initial subcategory if we are in edit mode
        const finalSubcategory = document.getElementById('finalSubcategoryInput');
        if (finalSubcategory && finalSubcategory.value) {
            // Small timeout to allow other elements to settle
            setTimeout(() => {
                selectSubcategory(finalSubcategory.value);
            }, 100);
        }

        // ===== CATEGORY FUNCTIONS =====
        window.showCategoryMode = function(mode) {
            const selectMode = document.getElementById('selectCategoryMode');
            const newMode = document.getElementById('newCategoryMode');
            const tabs = document.querySelectorAll('.category-tab');

            if (!selectMode || !newMode || !tabs.length) return;
        ...
        window.loadSubcategories = function() {
            ...
            if (!categoryName) {
                const quickList = document.getElementById('subcategoryQuickList');
                const subcatSelect = document.getElementById('subcategorySelect');
                if (quickList) quickList.style.display = 'none';
            ...
            if (category && category.subcategories && category.subcategories.length > 0) {
                ...
                const subcatSelect = document.getElementById('subcategorySelect');
                if (subcatSelect) subcatSelect.innerHTML = options;

                // Populate quick select list
                let quickListHtml = '';
                category.subcategories.forEach(sub => {
                    quickListHtml += '<span class="subcategory-item" onclick="selectSubcategory(\'' + sub.name + '\')">' + sub.name + '</span>';
                });
                const subcatList = document.getElementById('subcategoryList');
                if (subcatList) subcatList.innerHTML = quickListHtml;

                const quickListDiv = document.getElementById('subcategoryQuickList');
                if (quickListDiv) quickListDiv.style.display = 'block';

                // Update hidden inputs
                const finalCategory = document.getElementById('finalCategoryInput');
                if (finalCategory) finalCategory.value = categoryName;
            } else {
                const subcatSelect = document.getElementById('subcategorySelect');
                if (subcatSelect) subcatSelect.innerHTML = '<option value="">No subcategories</option>';
                const quickListDiv = document.getElementById('subcategoryQuickList');
                if (quickListDiv) quickListDiv.style.display = 'none';
            }
        };

        window.selectSubcategory = function(subName) {
            const subcatSelect = document.getElementById('subcategorySelect');
            const finalSubcat = document.getElementById('finalSubcategoryInput');

            if (subcatSelect) subcatSelect.value = subName;
            if (finalSubcat) finalSubcat.value = subName;

            // Highlight selected subcategory
            document.querySelectorAll('.subcategory-item').forEach(el => {
                el.classList.remove('selected');
                if (el.textContent === subName) {
        ...
        window.updateSubcategory = function() {
            ...
            if (select && finalSubcat) {
                finalSubcat.value = select.value;

                // Highlight in quick list
                document.querySelectorAll('.subcategory-item').forEach(el => {
                    el.classList.remove('selected');
        ...
        window.suggestCategory = function(category) {
        ...
        window.addNewCategory = function() {
            ...
            if (select) {
                ...
                option.setAttribute('data-id', Date.now());
            ...
            // Update hidden input
            const finalCategory = document.getElementById('finalCategoryInput');
            if (finalCategory) finalCategory.value = newCategory;

            const categoryId = document.getElementById('categoryIdInput');
            if (categoryId) categoryId.value = Date.now();

            alert('Category "' + newCategory + '" added! You can now add subcategories below.');

            // Switch to select mode
            showCategoryMode('select');
        ...
        window.addSubcategoryToList = function() {
            ...
            // Update hidden input
            const hidden = document.getElementById('newSubcategoriesHidden');
            if (hidden) hidden.value = JSON.stringify(newSubcategories);

            // Update display
            const list = document.getElementById('newSubcategoriesList');
            if (list) {
                let html = '';
                newSubcategories.forEach((s, index) => {
                    html += '<span style="display: inline-flex; align-items: center; gap: 5px; background: #e9ecef; padding: 5px 10px; border-radius: 20px; margin-right: 5px; margin-bottom: 5px;">' +
                        s +
                        '<button type="button" onclick="removeNewSubcategory(' + index + ')" style="background: none; border: none; color: #dc3545; cursor: pointer; font-size: 16px; font-weight: bold;">×</button>' +
                        '</span>';
                });
                list.innerHTML = html;
            }
        ...
        window.removeNewSubcategory = function(index) {
            ...
            if (list) {
                let html = '';
                newSubcategories.forEach((s, i) => {
                    html += '<span style="display: inline-flex; align-items: center; gap: 5px; background: #e9ecef; padding: 5px 10px; border-radius: 20px; margin-right: 5px; margin-bottom: 5px;">' +
                        s +
                        '<button type="button" onclick="removeNewSubcategory(' + i + ')" style="background: none; border: none; color: #dc3545; cursor: pointer; font-size: 16px; font-weight: bold;">×</button>' +
                        '</span>';
                });
                list.innerHTML = html;
            }
        ...
        window.calculateFinalPrice = function() {
            ...
            if (originalDisplay) originalDisplay.textContent = '₹' + originalPrice.toFixed(2);
            if (finalDisplay) finalDisplay.textContent = '₹' + finalPrice.toFixed(2);

            if (discountBadge) {
                if (discountType !== 'none' && discountValue > 0 && finalPrice < originalPrice) {
        ...
        // ===== SIZE-WISE STOCK FUNCTIONS =====
        window.addSizeRow = function(size, stock) {
            const tbody = document.getElementById('sizeStockBody');
            if (!tbody) {
                console.error('Size table body not found');
                return false;
            }

            // Check if size already exists
            const existingRows = document.querySelectorAll('input[name="size_names[]"]');
            for (let row of existingRows) {
                if (row.value.toUpperCase() === size.toUpperCase()) {
            ...
            newRow.innerHTML =
                '<td>' +
                '<input type="text" name="size_names[]" value="' + size.toUpperCase() + '" class="form-input" style="width: 100px;" readonly>' +
                '</td>' +
                '<td>' +
                '<input type="number" name="size_stocks[]" value="' + stock + '" class="form-input" min="0" style="width: 100px;" onchange="updateSizeStatus(this)">' +
                '</td>' +
        ...
        window.addSizeWithStock = function() {
            console.log('addSizeWithStock called');
            ...
            if (!sizeInput || !stockInput) {
                alert('Size input fields not found');
                return;
            }
        ...
        window.addNewSizeRow = function() {
            console.log('addNewSizeRow called');
            const sizeInput = document.getElementById('newSizeInput');
            if (sizeInput) {
                sizeInput.focus();
            } else {
                // Fallback if inputs don't exist
                const size = prompt('Enter size:');
                if (size) {
                    const stock = prompt('Enter stock quantity for size ' + size + ':', '0');
                    if (stock !== null) {
                        addSizeRow(size, parseInt(stock) || 0);
                    }
                }
            }
        };

        window.updateSizeStatus = function(input) {
            ...
            if (statusCell) {
                statusCell.className = 'stock-badge ' + (stock > 0 ? 'badge-in-stock' : 'badge-out-stock');
                statusCell.textContent = stock > 0 ? 'In Stock' : 'Out of Stock';
            }

            updateTotalStock();
        };

        window.deleteSizeRow = function(button) {
        ...
        window.updateTotalStock = function() {
            ...
            const overallStock = document.getElementById('overallStock');
            const totalStockValue = document.getElementById('totalStockValue');

            if (overallStock) overallStock.value = totalStock;
            if (totalStockValue) totalStockValue.textContent = totalStock;
        };
                ...
                files.forEach(file => {
                    ...
                    reader.onload = function(e) {
                        ...
                        div.innerHTML =
                            ...
                            '<button type="button" style="position: absolute; top: 5px; right: 5px; background: #dc3545; color: white; border: none; border-radius: 50%; width: 25px; height: 25px; cursor: pointer;" onclick="this.parentElement.remove()">×</button>';
        ...
        if (productForm) {
            productForm.addEventListener('submit', function(e) {
                // Make sure new_subcategories is set
                const newSubcatsHidden = document.getElementById('newSubcategoriesHidden');
                if (newSubcategories.length > 0 && newSubcatsHidden) {
                    newSubcatsHidden.value = JSON.stringify(newSubcategories);
                }
                ...
                const sizeInputs = document.querySelectorAll('input[name="size_names[]"]');
        ...
        // Run initial calculations after all elements are ready
        updateTotalStock();
        calculateFinalPrice();
...
// ============================================
// ============================================
// ADD/EDIT PRODUCT PROCESSING (FIXED SUBCATEGORY SAVING)
// ============================================
...
app.post("/add-product", upload.array("image", 5), (req, res) => {
  if (!req.session.loggedIn) {
    return res.redirect("/login");
  }
  
  const posts = loadPosts();
  const categories = loadCategories(); // Load existing categories
  ...
  const discountValue = parseFloat(req.body.discount_value) || 0;
...
// ===== FIXED: Handle new category and subcategories =====
// Check if we're adding a new category with subcategories
if (req.body.new_subcategories) {
  try {
    const newSubcats = JSON.parse(req.body.new_subcategories);
    
    // Only proceed if category name exists
    if (req.body.category && req.body.category.trim()) {
      // Create new category object
      const newCategory = {
        id: Date.now(),
        name: req.body.category,
        slug: slugify(req.body.category), // This is safe now with the fixed function
        subcategories: newSubcats.map((sub, index) => ({
          id: Date.now() + index + 1,
          name: sub,
          slug: slugify(sub) // This is safe now
        }))
      };
      ...
      console.log("New category added:", newCategory); // For debugging
    }
  } catch (e) {
...
// ===== END OF FIXED SECTION =====
  ...
  const productData = {
    ...
    showInNewArrivals: req.body.showInNewArrivals === "on", // ADD THIS LINE
    showInBestSellers: req.body.showInBestSellers === "on", // ADD THIS LINE
...
// ============================================
// SECTION 10: UPDATE LOGOUT ROUTE (FIND THIS ROUTE - around line 600)
...
app.post("/delete-post", (req, res) => {
  ...
  const { slug } = req.body; // Use slug instead of index
  const posts = loadPosts();
  const postIndex = posts.findIndex(p => p.slug === slug);
  
  if (postIndex >= 0) {
    // Optional: Check if user owns this post
...
// Helper function to calculate total stock from sizeStock
...
function adjustColor(hex, percent) {
  // Default fallback colors if hex is invalid
  ...
  // If hex is invalid, return fallback
...
// ============================================
// ADMIN HELPER FUNCTIONS (ADD THESE)
// ============================================



// Helper function to get admin footer HTML
...
// ADMIN DASHBOARD PAGE (ADD HERE - BEFORE PRODUCT REVIEWS)
// ============================================
...
// ============================================
// ============================================
// PRODUCT DETAIL PAGE (FIXED WITH MULTIPLE IMAGES)
// ============================================
  ...
  <div class="product-detail-container">
    ...
    <div class="product-main">
      ...
      <div class="product-images">
        ...
        <!-- Thumbnail Images - FIXED: Yeh sab images show karega -->
        ...
        ` : product.images && product.images.length === 1 ? `
        <!-- Sirf 1 image hai toh kuch nahi dikhana -->
        ` : ''}
...
` : ''}
  ...
  <script>
    ...
    // FIXED: Thumbnail click function
    ...
    // FIXED: Console log to check images
    document.addEventListener('DOMContentLoaded', function() {
...
<body>
  ...
  <div class="analytics-container">
   ...
   // ============================================
// ADMIN ANALYTICS PAGE - REPLACE THE METRICS SECTION
// ============================================

<!-- Key Metrics - Replace this entire section -->
<div class="analytics-grid">
   ...
   <!-- Performance Metrics - Replace this section -->
...
// ============================================
// ADMIN CATEGORIES PAGE - ADD THIS HERE
// ============================================
...
<body>
  ...
  <script>
    ...
    // Auto-fill address if user is logged in (simplified)
    ${user && user.address ? `
    document.addEventListener('DOMContentLoaded', function() {
...
// ============================================
// ============================================
// SECTION 8: UPDATE YOUR LOGIN ROUTE (FIND THIS ROUTE - around line 2550)
// REPLACE YOUR EXISTING /login-user POST route with this
// ============================================

app.post("/login-user", authLimiter, async (req, res) => {
  ...
  // Check if password is using old base64 or new bcrypt
  let isValidPassword = false;
...
// ============================================
// SECTION 7: UPDATE YOUR REGISTER ROUTE (FIND THIS ROUTE - around line 2500)
// REPLACE YOUR EXISTING /register POST route with this
// ============================================
...
app.post("/register", [
  ...
  const newUser = {
    ...
    email,
    phone: phone || '',
    password: hashedPassword, // Now using bcrypt
    createdAt: new Date().toISOString(),
    isActive: true,
    role: 'user'
  };
...
<body>
  ...
  <div class="track-container">
    ...
    <div class="timeline-container">
      ...
      <div class="timeline">
        ${trackingEvents.length > 0 ? trackingEvents.map(event => `
        ...
        <div class="timeline-event">
          ...
          <div class="timeline-content">
            ...
            <div class="timeline-description">Your order has been placed</div>
            <div class="timeline-date">${order.date ? new Date(order.date).toLocaleString() : 'N/A'}</div>
          </div>
        </div>
        `}
      </div>
  ...
  <script>
    ...
    // Auto-refresh tracking data every 30 seconds (optional)
    // setInterval(() => {
    //   location.reload();
    // }, 30000);
  </script>
...
// OTHER ADMIN PAGES (ADD HERE - AFTER PRODUCT REVIEWS)
// ============================================
...
<body>
    ...
    <div class="content-card">
        <div class="card-header">
            <h2 class="card-title"><i class="fas fa-box"></i> Products Management</h2>
            <a href="/add-product" class="btn btn-primary">
                <i class="fas fa-plus"></i> Add Product
            </a>
        </div>
        
        <p>Total Products: ${posts.length}</p>
        ...
        <table class="orders-table">
            ...
            <tbody>
                ${filteredOrders.map(order => {
                    const orderDate = order.date ? new Date(order.date).toLocaleDateString() : new Date(order.id || Date.now()).toLocaleDateString();
                    const itemCount = order.items ? order.items.reduce((sum, item) => sum + (item.quantity || 1), 0) : 0;
                    
                    return `
                    <tr>
                        <td style="font-weight: 600;">#${order.id || 'N/A'}</td>
                        <td>
                            <div>${orderDate}</div>
                            <small style="color: #666;">${order.date ? new Date(order.date).toLocaleTimeString() : ''}</small>
                        </td>
                        <td>
                        ...
                        <td style="font-weight: 600; color: #e53935; font-size: 18px;">₹${order.total || 0}</td>
                        <td>
                            <span class="status-badge ${order.paymentStatus === 'completed' ? 'status-completed' : 'status-pending'}">
                        ...
                        <td>
                            <select class="status-badge" onchange="updateOrderStatus('${order.id}', this.value)" 
                                    style="padding: 5px; border-radius: 20px; border: 2px solid #ddd;">
...
// Admin Analytics Page (you already have this)
app.get("/admin/analytics", (req, res) => {
  // ... your existing analytics code ...
});

// Admin Customers Page (you already have this)
app.get("/admin/customers", (req, res) => {
  // ... your existing customers code ...
});
...
// ============================================
// SECTION 12: UPDATE SERVER START (FIND THIS AT THE VERY END)
// REPLACE YOUR EXISTING server.listen with this
// ============================================
...
if (process.env.NODE_ENV === 'production') {
  ...
  try {
    ...
    https.createServer(options, app).listen(PORT, () => {
      ...
      console.log(`✅ Security features enabled: Helmet, Rate Limiting, CORS, Input Sanitization`);
    });
  } catch (error) {
    console.error('Failed to start HTTPS server:', error);
    console.log('Falling back to HTTP server...');
    app.listen(PORT, () => {
      console.log(`⚠️  HTTP Server running on port ${PORT} (Not secure for production)`);
      console.log(`✅ Security features enabled: Helmet, Rate Limiting, CORS, Input Sanitization`);
    });
  }
} else {
  ...
  app.listen(PORT, () => {
    ...
    console.log(`✅ Security features enabled: Helmet, Rate Limiting, CORS, Input Sanitization`);
    console.log(`⚠️  Passwords are now hashed with bcrypt`);
  });
}
// ============================================
    ...
    <div class="admin-header">
        ...
        <div class="header-actions">
            <div class="user-info">
                ...
                <div>
                    <div style="font-weight: 600;">${req.session.userName || 'Admin'}</div>
                    <small style="color: #aaa;">Super Administrator</small>
                </div>
            </div>
            <a href="/" class="btn btn-outline" style="color: white; border-color: white;">
                <i class="fas fa-globe"></i> View Site
            </a>
    ...
    <div class="admin-container">
        ...
        <div class="sidebar">
            ...
            <div class="sidebar-menu">
                ...
                <a href="/admin/sliders" class="menu-item">
                    <i class="fas fa-images"></i>
                    <span>Sliders</span>
                </a>
...
<div class="stats-grid">
    <div class="stat-card">
        ...
        <div class="stat-value">₹${orders.length > 0 ? orders.reduce((sum, order) => sum + (parseFloat(order.total) || 0), 0).toLocaleString() : '0'}</div>
        <div class="stat-label">Total Revenue</div>
        <div class="stat-change positive">
            <i class="fas fa-arrow-up"></i>
            <span>${orders.length > 0 ? '12%' : '0%'} from last month</span>
        </div>
    </div>
    ...
    <div class="stat-card">
        ...
        <div class="stat-change positive">
            <i class="fas fa-arrow-up"></i>
            <span>${orders.length > 0 ? '8%' : '0%'} from last month</span>
        </div>
    </div>
    ...
    <div class="stat-card">
        ...
        <div class="stat-change positive">
            <i class="fas fa-arrow-up"></i>
            <span>${posts.length > 0 ? '5%' : '0%'} from last month</span>
        </div>
    </div>
    ...
    <div class="stat-card">
        ...
        <div class="stat-change positive">
            <i class="fas fa-arrow-up"></i>
            <span>${users.length > 0 ? '15%' : '0%'} from last month</span>
        </div>
    </div>
</div>
<!-- Quick Stats -->
<div class="quick-stats">
    <div class="quick-stat">
        <div class="quick-stat-icon" style="background: #667eea;">
            <i class="fas fa-box"></i>
        </div>
    ...
    <div class="quick-stat">
        <div class="quick-stat-icon" style="background: #f5576c;">
            <i class="fas fa-shopping-cart"></i>
        </div>
        <div class="quick-stat-info">
            <h3>${orders.length}</h3>
    ...
    <div class="quick-stat">
        <div class="quick-stat-icon" style="background: #4facfe;">
            <i class="fas fa-images"></i>
        </div>
        <div class="quick-stat-info">
    ...
    <div class="quick-stat">
        <div class="quick-stat-icon" style="background: #43e97b;">
            <i class="fas fa-rupee-sign"></i>
        </div>
        <div class="quick-stat-info">
            <h3>₹${orders.length > 0 ? orders.reduce((sum, order) => sum + (parseFloat(order.total) || 0), 0).toLocaleString() : '0'}</h3>
            <p>Total Revenue</p>
        </div>
    </div>
</div>
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div style="font-weight: 600;">${order.customerName || 'Guest'}</div>
                                    <small style="color: var(--gray);">${order.email || ''}</small>
                                </td>
                                <td>${order.date || new Date().toLocaleDateString()}</td>
                                <td style="font-weight: 600; color: var(--primary);">₹${order.total || 0}</td>
                                <td>
                                    <span class="status-badge status-active">Completed</span>
                                </td>
                                <td>
            ...
            <div class="content-card">
                ...
                <div class="table-responsive">
                    <table class="table">
                        ...
                        <tbody>
                            ${posts.filter(p => (p.stock || 0) < 10).slice(0, 5).map((product, i) => `
                            <tr>
                                <td>
                                    <div style="display: flex; align-items: center; gap: 10px;">
                                        ${product.images && product.images[0] ? `
                                        <img src="${product.images[0]}" class="product-image">
                                        ` : `
                                        <div style="width: 60px; height: 60px; background: var(--light-gray); border-radius: 8px; display: flex; align-items: center; justify-content: center;">
                                            <i class="fas fa-box" style="color: var(--gray);"></i>
                                        </div>
                                        `}
                                        <div>
                                            <div style="font-weight: 600;">${product.name}</div>
                                            <small style="color: var(--gray);">SKU: ${product.slug}</small>
                                        </div>
                                    </div>
                                </td>
                                <td>${product.category || 'General'}</td>
                                <td>
                                    <div style="font-weight: 600; color: ${(product.stock || 0) < 5 ? 'var(--danger)' : 'var(--warning)'};">${product.stock || 0}</div>
                                </td>
                                <td style="font-weight: 600;">₹${product.price || 0}</td>
                                <td>
                                    <span class="status-badge ${(product.stock || 0) > 0 ? 'status-active' : 'status-inactive'}">
                            ...
                            ${posts.filter(p => (p.stock || 0) < 10).length === 0 ? `
                            <tr>
                                <td colspan="6" style="text-align: center; padding: 40px; color: var(--gray);">
                                    <i class="fas fa-check-circle" style="font-size: 40px; margin-bottom: 15px; color: var(--success); opacity: 0.3;"></i>
                                    <h3>All Products in Stock</h3>
                                    <p>All your products have sufficient stock levels.</p>
                                </td>
                            </tr>
            ...
            <div class="content-card">
                ...
                <div style="padding: 20px;">
                    <div style="border-left: 2px solid var(--light-gray); padding-left: 20px;">
                        <div style="margin-bottom: 25px; position: relative;">
                            <div style="position: absolute; left: -28px; top: 0; width: 10px; height: 10px; background: var(--primary); border-radius: 50%; border: 3px solid white;"></div>
                            <div style="font-weight: 600;">New order received</div>
                            <div style="color: var(--gray); font-size: 14px;">Order #${orders.length > 0 ? orders[orders.length - 1]?.id : '1001'} - ${new Date().toLocaleString()}</div>
                        </div>
                        
                        <div style="margin-bottom: 25px; position: relative;">
                            <div style="position: absolute; left: -28px; top: 0; width: 10px; height: 10px; background: var(--success); border-radius: 50%; border: 3px solid white;"></div>
                            <div style="font-weight: 600;">New product added</div>
                            <div style="color: var(--gray); font-size: 14px;">"${posts.length > 0 ? posts[posts.length - 1]?.name : 'New Product'}" - ${new Date().toLocaleDateString()}</div>
                        </div>
                        
                        <div style="margin-bottom: 25px; position: relative;">
                            <div style="position: absolute; left: -28px; top: 0; width: 10px; height: 10px; background: var(--info); border-radius: 50%; border: 3px solid white;"></div>
                            <div style="font-weight: 600;">New customer registered</div>
                            <div style="color: var(--gray); font-size: 14px;">${users.length > 0 ? users[users.length - 1]?.name : 'New User'} - ${new Date().toLocaleDateString()}</div>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        // Simple tab switching for demo purposes
        function showTab(tabId) {
            alert('Navigating to ' + tabId + ' section');
            // In a real implementation, this would load the corresponding content
            // or redirect to the appropriate admin page
        }
    </script>
</body>
</html>`);
...
// SECTION 11: ADD ERROR HANDLING MIDDLEWARE (AT THE END, BEFORE SERVER START)
// ============================================

// 404 handler - add this before your server start
app.use((req, res, next) => {
  res.status(404).send(`
    <!DOCTYPE html>
    <html>
...
// Global error handler - add this before server start
app.use((err, req, res, next) => {
  ...
  if (err instanceof multer.MulterError) {
    ...
    if (err.code === 'LIMIT_UNEXPECTED_FILE') {
      return res.status(400).send('Unexpected field name.');
    }
