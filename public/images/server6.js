...
const fileFilter = (req, file, cb) => {
  // Check magic numbers (file signatures) not just extension
  const allowedTypes = ['image/jpeg', 'image/jpg', 'image/png'];
  
  // Additional check: verify file signature
  const buffer = file.buffer.slice(0, 4);
  const header = buffer.toString('hex');
  const isJPEG = header.startsWith('ffd8ffe0') || header.startsWith('ffd8ffe1');
  const isPNG = header.startsWith('89504e47');
  
  if (allowedTypes.includes(file.mimetype) && (isJPEG || isPNG)) {
    cb(null, true);
  } else {
...
// ============================================
// ============================================
// ADD/EDIT PRODUCT PAGE - COMPLETELY FIXED
// ============================================
...
<body>
  ...
  <div class="product-form-container">
    ...
    <div class="form-card">
      <form method="POST" action="/add-product" enctype="multipart/form-data" id="productForm">
        ...
        <!-- Size-wise Stock Management - FIXED VERSION -->
        ...
        <div class="form-group">
          ...
          <div class="file-upload">
            <div class="file-upload-label" onclick="document.getElementById('images').click()">
              <i class="fas fa-cloud-upload-alt"></i>
              <span>Click to upload images (max 5)</span>
            </div>
            <input type="file" name="images" id="images" accept="image/*" multiple 
                   ${!isEditMode ? 'required' : ''} onchange="previewImages(this)">
          </div>
  ...
  <!-- FIXED JAVASCRIPT - This will make everything work -->
  <script>
    // Wait for DOM to load - IMPORTANT!
    document.addEventListener('DOMContentLoaded', function() {
      console.log('Add Product Page Loaded - All systems go!');
      
      // Calculate initial total stock
      updateTotalStock();
    });
    
    // ===== SIZE FUNCTIONS - These will work now =====
    function addSize() {
      ...
      newRow.innerHTML = 
        ...
        '<td><input type="number" name="size_stocks[]" value="' + stock + '" class="form-input" min="0" style="width: 80px;" onchange="updateTotalStock()"></td>' +
        '<td><span class="stock-badge ' + (stock > 0 ? 'badge-in-stock' : 'badge-out-stock') + '">' + (stock > 0 ? 'In Stock' : 'Out of Stock') + '</span></td>' +
        '<td><button type="button" class="delete-size-btn" onclick="removeSizeRow(this)"><i class="fas fa-trash"></i> Delete</button></td>';
    ...
    function updateTotalStock() {
      ...
      document.querySelectorAll('input[name="size_stocks[]"]').forEach(input => {
        ...
        if (row) {
          ...
          if (badge) {
            badge.className = 'stock-badge ' + (stock > 0 ? 'badge-in-stock' : 'badge-out-stock');
            badge.textContent = stock > 0 ? 'In Stock' : 'Out of Stock';
          }
        }
      });
    }
...
// Wait for DOM to load - IMPORTANT!
document.addEventListener('DOMContentLoaded', function() {
    console.log('Add Product Page Loaded - All functions ready!');
    
    // Initialize - update total stock if any sizes exist
    updateTotalStock();
});

// ===== SIZE FUNCTIONS =====
function addSize() {
    console.log('Add Size clicked - function called');
    ...
    newRow.innerHTML = 
        ...
        '<td><input type="number" name="size_stocks[]" value="' + stock + '" class="form-input" min="0" style="width: 80px;" onchange="updateTotalStock()"></td>' +
        '<td><span class="stock-badge ' + (stock > 0 ? 'badge-in-stock' : 'badge-out-stock') + '">' + (stock > 0 ? 'In Stock' : 'Out of Stock') + '</span></td>' +
        '<td><button type="button" class="delete-size-btn" onclick="removeSize(this)"><i class="fas fa-trash"></i> Delete</button></td>';
...
function calculateDiscount() {
    ...
    if (finalSpan) finalSpan.textContent = finalPrice.toFixed(2);
}
    ...
    if (input.files) {
        ...
        files.forEach(file => {
            ...
            reader.onload = function(e) {
                ...
                div.innerHTML = 
                    '<img src="' + e.target.result + '" style="width: 100px; height: 100px; object-fit: cover; border-radius: 5px; border: 2px solid #ddd;">' +
                    '<button type="button" style="position: absolute; top: 5px; right: 5px; background: #dc3545; color: white; border: none; border-radius: 50%; width: 25px; height: 25px; cursor: pointer;" onclick="this.parentElement.remove()">×</button>';
                preview.appendChild(div);
            };
            reader.readAsDataURL(file);
